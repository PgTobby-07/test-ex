...
import { getProductFilterOptions, getProducts } from "@/services/productService";
...
export default async function ProductsPage({
    ...
    const search = normalizeSingle(params.search)?.trim();
    const categorySlug = normalizeSingle(params.category_slug)?.trim();
    const minPriceRaw = normalizeSingle(params.min_price)?.trim();
    const maxPriceRaw = normalizeSingle(params.max_price)?.trim();
    const pageRaw = normalizeSingle(params.page)?.trim();
    const attributes = normalizeMany(params.attributes).filter(Boolean);
    const inStock = normalizeSingle(params.in_stock) === "true";
    ...
    const buildPageHref = (nextPage: number) => {
        ...
        if (categorySlug) url.set("category_slug", categorySlug);
        ...
        <main className="min-h-screen bg-background px-4 py-10">
            <section className="mx-auto max-w-7xl">
                <div className="grid gap-8 lg:grid-cols-[280px_minmax(0,1fr)]">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold text-foreground">{t("products_title")}</h1>
                        {search ? (
